...
async fn test_fetch_grugbrain() {
    ...
    // Extract the file path from the result message (new format)
    // The new format is: "Content downloaded successfully\n\nFile: <path>\nSize: ...\n..."
...
async fn test_file_path_is_relative() {
    ...
    // Extract the file path from the result (new format)
    let result_text = result.content
        .iter()
        .filter_map(|c| {
            if let rmcp::model::RawContent::Text(t) = &c.raw {
